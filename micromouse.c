...
unsigned char mazeflood[36]={ // mang luu tru gia tri flood fill, ban đầu tạo sẵn mảng theo cấu trúc mê cung
6, 5, 4, 3, 4, 5,
5, 4, 3, 2, 3, 4,
4, 3, 2, 1, 2, 3,
3, 2, 1, 0, 1, 2,
...
led=1;
   ...
   led=1;
   movestraight(); 
   // tương ứng đang xét ô 6 x 6
   if (current_dir==1) current_cell+=6; // huong North // lưu mảng 1 chiều nên + 6 
   ...
   destination=21; // đích đến
   while (true)
      {
        if (speed<2)
        {
      read_sensor();
         if (current_dir==1)// hướng di chuyển lên trên
         {
               // cap nhat buc tuong cell hien tai
               // kiểm tra xem ô hiện tại đã xét tường chưa, nếu chưa thì xét và lưu 1 vào bit 7 ( đã xét tường ), và xét tới 3 hướng
...
void flood_fill()
{
   ...
   int stk[128], stkptr = 0, stk_empty_flag = 0; // mảng stack gồm 128 ô, con trỏ stkptr, set flag = 0 khi stack chưa rỗng, 1 khi rỗng  
   ...
   while (stk_empty_flag == 0) 
   {
      ...
      // floodcell = current cell, mazeflood như 1 mảng mới để check tường
      // xét nếu ko có bức tường cản trở thì coi như là ô hàng xóm
      if (!(CHECKBIT(wallinfo, BIT0) ) )  // kiểm tra bức tường hướng Bắc
         neighbour_val[0] = mazeflood[floodcell + 6]; // gán giá trị của neighbor vào để xét

      //East
      if (!(CHECKBIT(wallinfo, BIT1) ) )   // kiểm tra bức tường hướng Đông, bên phải
      ...
      //West
      if (!(CHECKBIT(wallinfo, BIT3) ) )   // kiểm tra bức tường hướng Tây, bên trái
         neighbour_val[3] = mazeflood[floodcell - 1]; 

// sau khi gán giá trị Flood các ô hàng xóm vào các biến tương ứng ta thực hiện kiểm tra giá trị nhỏ nhất trong 4 ô hàng xóm:
// quy về ô nhỏ nhất sẽ ở vị trí neightbour_val 0
      x = 0;
      while (x < 3)
      {
         if (neighbour_val[0] > (neighbour_val[x + 1]))
            neighbour_val[0] = neighbour_val[x + 1];// ổ nhỏ nhất

         x+=1;
      }      
   // Nếu giá trị cell hiện tại khác với đích tới và giá trị ô hiện tại không = ô hàng xóm + 1  thì update ô hiện tại lên giá trị = ô hàng xóm + 1 
      if ((!(floodcell == destination )) && (!(mazeflood[floodcell] == (1+neighbour_val[0]))))

      {
         mazeflood[floodcell] = 1 + neighbour_val[0];         // update giá trị lên + 1 so với ô hàng xóm

         //North
         if (!(CHECKBIT(wallinfo, BIT0))) // kiểm tra nếu ko có tường phía bắc thì đẩy giá trị ô hàng xóm phía bắc vào stack để flood_fill
         {
            stkptr++; 
         ...
         //East
         if (!(CHECKBIT(wallinfo, BIT1)))// tương tự cho ô phía Đông
         {
            stkptr++;
         ...
         //South
         if (!(CHECKBIT(wallinfo, BIT2)))// tương tự cho ô phía Nam
         {
            stkptr++;
            stk[stkptr] = floodcell - 6;
            stk_empty_flag = 0;  
         ...
         //West
         if (!(CHECKBIT(wallinfo, BIT3))) // tương tự cho ô phía Tây
         {
            stkptr++;
            stk[stkptr] = floodcell - 1;
            stk_empty_flag = 0;   
...
// gán các giá trị của ô hàng xóm sau khi đã flood_fill cho neighbour để xét hướng
   ...
   // tìm giá trị ô hàng xóm bé nhất
   x = 0;
   x2 = 0;
   while (x < 3)
   {
      ...
      {
         x2 = x + 1;
         neighbour_val[4] = pow(2, ( 2 * x2 )) ; // đầu tiên val 4 có giá trị là 1 là hướng Bắc, nếu các ô cập nhật bé hơn thì cập nhật các giá trị khác như 4, 16, 64 để xác định hướng
         neighbour_val[0] = neighbour_val[x + 1];

      }
      x += 1;
   }  
  // current_dir ban đầu luôn bằng 1
  // xét đầu xe và ô hàng xóm bé nhất để quẹo
   if (current_dir == neighbour_val[4]) // nếu trùng hướng thì đi thẳng
   {
      neighbour_val[4] = 'M'; // đi thẳng
      return neighbour_val[4];
   }
   // ví dụ dir == 64 và neigh == 1 thì là xe hướng W, ô bé nhất hướng N -> quẹo phải
   if ( ((current_dir == 64) && (neighbour_val[4] == 1)) || (current_dir == (neighbour_val[4]/4)) )
   {
      neighbour_val[4] = 'R';
      return neighbour_val[4];
   }
   // lập luận tương tự.
   if ( ((current_dir == 1) && (neighbour_val[4] == 64)) || (current_dir == (4 * neighbour_val[4])) )
   {
      neighbour_val[4] = 'L';
      return neighbour_val[4];
   }
